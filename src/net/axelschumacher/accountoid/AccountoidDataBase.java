...
public class AccountoidDataBase {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase database) {
			database.execSQL("CREATE TABLE " + ACCOUNT_TABLE_NAME + " ("
					+ Account._ID + " INTEGER PRIMARY KEY," + Account.CATEGORY
					+ " INTEGER," + Account.CURRENCY + " INTEGER,"
					+ Account.DATE + " INTEGER," + Account.STATE + " INTEGER,"
					+ Account.DESCRIPTION + " TEXT," + Account.AMOUNT
					+ " FLOAT" + ");");
			...
			database.execSQL("CREATE TABLE " + CURRENCIES_TABLE_NAME + " ("
					...
					+ Currencies.CODE + " TEXT UNIQUE," + Currencies.VALUE
					+ " FLOAT" + ");");

			// Insert at least the local currency
			ContentValues value = new ContentValues();
			value.put(Currencies.CODE, Currency
	...
	public Cursor getAccountList() {
		...
		String projection[] = { Account._ID, Account.AMOUNT,
				Account.DESCRIPTION, Account.CURRENCY, Account.DATE };

		openDataBase();
		Cursor c = qb.query(database, projection, null, null, null, null,
				orderBy);
	...
	public boolean deleteAccount(long id) {
		openDataBase();
		int count;
		count = database.delete(ACCOUNT_TABLE_NAME, Account._ID + "=" + id,
				null);
	...
	public Uri insertAccount(ContentValues initialValues) {
		...
		if (rowId > 0) {
			Uri noteUri = ContentUris
					.withAppendedId(Account.CONTENT_URI, rowId);
			return noteUri;
		}
	...
	public boolean updateAccount(long id, ContentValues values) {
		openDataBase();
		int count;
		count = database.update(ACCOUNT_TABLE_NAME, values, Account._ID + "="
				+ id, null);
	...
	public void updateCurrencyRate(long id, ContentValues values) {
		openDataBase();
		database.update(CURRENCIES_TABLE_NAME, values, Currencies._ID + "="
				+ id, null);
	}
	...
	public Uri insertCategory(ContentValues initialValues) {
		...
		if (rowId > 0) {
			Uri catUri = ContentUris.withAppendedId(Categories.CONTENT_URI,
					rowId);
			return catUri;
		}
	 ...
	 *             if {@link Currencies}.CODE field is not defined
	 * @throws SQLException
	...
	public Uri insertCurrency(ContentValues initialValues)
			throws SQLiteConstraintException, IllegalArgumentException {
		ContentValues values;
		if (initialValues != null)
			values = new ContentValues(initialValues);
		else
		...
		Currency.getInstance(code); // Will throw an Illegal argument exception
									// if not valid
		...
		if (rowId > 0) {
			Uri curUri = ContentUris.withAppendedId(Currencies.CONTENT_URI,
					rowId);
			return curUri;
		}

		throw new SQLiteConstraintException("Failed to insert row into "
	...
	public Cursor getCurrencyCursorFromIndex(long index)
		...
		openDataBase();
		Cursor c = qb
				.query(database, projection, where, null, null, null, null);
	...
	public boolean deleteCurrency(long id) {
		...
		int count;
		count = database.delete(CURRENCIES_TABLE_NAME, Account._ID + "=" + id,
				null);
	...
	public boolean deleteCategory(long id) {
		SQLiteDatabase database = openHelper.getWritableDatabase();
		int count;
		count = database.delete(CATEGORIES_TABLE_NAME, Account._ID + "=" + id,
				null);
